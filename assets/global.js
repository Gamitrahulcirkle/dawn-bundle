...
class QuantityInput extends HTMLElement {
  ...
  renderedCallback(){
    console.log( "calling renderedCallback 21252121254121" );
  }
  ...
  onButtonClick(event) {
    ...
    if( this.input.value > 10  ){      
      if( window.location.href.indexOf("/cart") === -1 ){
        event.target.name === 'minus' ? this.input.value = this.input.value - 10: this.input.value;
      }else{
        if( this.input.value < 10 ){
          this.input.value = 0;
        }       
  ...
  validateQtyRules(event) {
    ...
    if( typeof this.input.dataset.maxQtyTrack !="undefined" && this.input.dataset.maxQtyTrack !="" ){
      let maxQty = parseInt( this.input.dataset.maxQtyTrack - 10 );
      for (var i = 10; i <= maxQty; i += 10) {
        if( i < value && i != value ){
          this.input.value = i+10;
        }else{
          if( value < 10 ){
            this.input.value = 10;
          }
    ...
    const handleIntersection = (entries, observer) => {
      ...
      fetch(this.dataset.url)
        ...
        .then((text) => {
          ...
          /* Create custom event for related products wishlist functionality (wishlist.js) */
